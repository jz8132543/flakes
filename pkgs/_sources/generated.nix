# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchurl,
  fetchFromGitHub,
}:
{
  alacritty-catppuccin = {
    pname = "alacritty-catppuccin";
    version = "f6cb5a5c2b404cdaceaff193b9c52317f62c62f7";
    src = fetchFromGitHub {
      owner = "catppuccin";
      repo = "alacritty";
      rev = "f6cb5a5c2b404cdaceaff193b9c52317f62c62f7";
      fetchSubmodules = false;
      sha256 = "sha256-H8bouVCS46h0DgQ+oYY8JitahQDj0V9p2cOoD4cQX+Q=";
    };
    date = "2024-10-28";
  };
  alist = {
    pname = "alist";
    version = "v4.1.9";
    src = fetchurl {
      url = "https://github.com/OpenListTeam/OpenList/releases/download/v4.1.9/openlist-linux-musl-amd64.tar.gz";
      sha256 = "sha256-Jgl7U0aTU1zhIq05Z0pCr6RTxrMVd/xKiqrkXG0TIpY=";
    };
  };
  clash-webui-yacd-meta = {
    pname = "clash-webui-yacd-meta";
    version = "ac485b00be400b6c6efa3c73a579ac419bc439ec";
    src = fetchurl {
      url = "https://github.com/MetaCubeX/Yacd-meta/archive/ac485b00be400b6c6efa3c73a579ac419bc439ec.zip";
      sha256 = "sha256-8lktBjGpDPXapKFWb68XuzpUe2DNpuMlfH6P7Ry7Af4=";
    };
    date = "2026-01-16";
  };
  custom-win10-fonts = {
    pname = "custom-win10-fonts";
    version = "4adc0a4198cc090b63bcb667363c03101e8175eb";
    src = fetchFromGitHub {
      owner = "jz8132543";
      repo = "ttf-ms-win10";
      rev = "4adc0a4198cc090b63bcb667363c03101e8175eb";
      fetchSubmodules = false;
      sha256 = "sha256-pymEG+Ygll+2YY8NyUj5Ky3ExRSlfFvthz5ap5scO4Y=";
    };
    date = "2025-01-01";
  };
  ebook-sender-bot = {
    pname = "ebook-sender-bot";
    version = "380de137064e0c92bbb99a38cbbeab06f425c5b1";
    src = fetchFromGitHub {
      owner = "qcgzxw";
      repo = "ebook-sender-bot";
      rev = "380de137064e0c92bbb99a38cbbeab06f425c5b1";
      fetchSubmodules = false;
      sha256 = "sha256-JXcnwwxXCc+aci4DU+YXAW2U+TMyTPBBE1vG0hon1mY=";
    };
    date = "2024-12-28";
  };
  jellyfin-plugin-intro-skipper = {
    pname = "jellyfin-plugin-intro-skipper";
    version = "1.10.11.14";
    src = fetchurl {
      url = "https://github.com/intro-skipper/intro-skipper/releases/download/10.11/v1.10.11.14/intro-skipper-v1.10.11.14.zip";
      sha256 = "sha256-aoSZq8rUesV8U6fjWkDLNkdjOviPdAxppGpfJz8e3f8=";
    };
  };
  jellyfin-plugin-reports = {
    pname = "jellyfin-plugin-reports";
    version = "18.0.0.0";
    src = fetchurl {
      url = "https://github.com/jellyfin/jellyfin-plugin-reports/releases/download/v18/reports_18.0.0.0.zip";
      sha256 = "sha256-4DIszP6GAQRQv9Ky10sC2HVEjRJRkPiZT3oqq1csI/c=";
    };
  };
  jellyfin-plugin-sso = {
    pname = "jellyfin-plugin-sso";
    version = "4.0.0.3";
    src = fetchurl {
      url = "https://github.com/9p4/jellyfin-plugin-sso/releases/download/v4.0.0.3/sso-authentication_4.0.0.3.zip";
      sha256 = "sha256-3glRJVvsTtZGA3ZB5+CqEhCzoAoUFAZUgIe+2ZTLm90=";
    };
  };
  kitty-catppuccin = {
    pname = "kitty-catppuccin";
    version = "43098316202b84d6a71f71aaf8360f102f4d3f1a";
    src = fetchFromGitHub {
      owner = "catppuccin";
      repo = "kitty";
      rev = "43098316202b84d6a71f71aaf8360f102f4d3f1a";
      fetchSubmodules = false;
      sha256 = "sha256-akRkdq8l2opGIg3HZd+Y4eky6WaHgKFQ5+iJMC1bhnQ=";
    };
    date = "2026-01-16";
  };
  trackerslist = {
    pname = "trackerslist";
    version = "latest";
    src = fetchurl {
      url = "https://cf.trackerslist.com/best_aria2.txt";
      sha256 = "sha256-fAS1YKR3JK7xBI3fgpZ5ZTbqZMOBSql6nB8lC8gCuQQ=";
    };
  };
}
