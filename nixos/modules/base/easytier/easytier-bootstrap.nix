...
let
  inherit (lib)
    ...
    types
    ;
  cfg = config.services.easytierMesh.bootstrap;
  envName = "easytier-bootstrap.env";
  instanceName = "mesh";
...
in
{
  options.services.easytierMesh.bootstrap = {
    enable = (mkEnableOption "EasyTier bootstrap node") // {
      default = true;
    };
    ...
    ipv4 = mkOption {
      ...
      default = null;
      # type = types.str;
      # default = "10.144.0.1/24";
      description = "Static EasyTier address for the bootstrap node.";
    };
  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      }
      {
        ...
        message = "services.easytierMesh.bootstrap.defaultProtocol must match an enabled protocol.";
      }
    ];
    ...
    services.traefik.proxies.easytier-bootstrap-rpc = mkIf (config.services.traefik.enable or false) {
      rule = "Host(`${config.networking.fqdn}`) && (Path(`/et`) || PathPrefix(`/et/`))";
      target = "http://${cfg.rpcPortal}";
      middlewares = [
        "auth"
        "easytier-bootstrap-rpc-stripprefix"
      ];
    };

    services.traefik.dynamicConfigOptions.http.middlewares.easytier-bootstrap-rpc-stripprefix.stripPrefix.prefixes =
      [
        "/et"
      ];
  };
