  ...
  config = lib.mkIf cfg.enable {
    ...
    boot.initrd.systemd.services.cloud-btrfs-swap-partition = {
      ...
      script = ''
        ...
        fstype="$(${pkgs.util-linux}/bin/lsblk -no FSTYPE "${rootDevice}" | head -n1 | tr -d '[:space:]')"
        ...
        if [ -z "$min_dev_size_bytes" ] || [ "$min_dev_size_bytes" -le 0 ]; then
          ...
          if [ "$fstype" = "btrfs" ]; then
            ...
            ${pkgs.e2fsprogs}/bin/e2fsck -f -p "${rootDevice}" || true
            ${pkgs.e2fsprogs}/bin/resize2fs "${rootDevice}" || true
          fi
          exit 0
        fi
        ...
        if [ "$fstype" = "btrfs" ]; then
          ...
          block_count="$(${pkgs.e2fsprogs}/bin/dumpe2fs -h "${rootDevice}" 2>/dev/null | ${pkgs.gawk}/bin/awk '/^Block count:/ {print $3; exit}')"
          if [ -n "$block_count" ]; then
            ...
            if [ "$current_ext4_bytes" -gt "$planned_root_bytes" ]; then
              ...
              ${pkgs.e2fsprogs}/bin/e2fsck -f -p "${rootDevice}" || true
              if ! ${pkgs.e2fsprogs}/bin/resize2fs "${rootDevice}" $((planned_root_bytes / block_size)); then
        ...
        if [ "$fstype" = "ext4" ]; then
          ...
          ${pkgs.e2fsprogs}/bin/e2fsck -f -p "${rootDevice}" || true
          if ! ${pkgs.e2fsprogs}/bin/resize2fs "${rootDevice}"; then
          ...
          mkdir -p "$btrfs_mount"
          if ${pkgs.util-linux}/bin/mount -t btrfs -o subvolid=5 "${rootDevice}" "$btrfs_mount"; then
            ${pkgs.btrfs-progs}/bin/btrfs filesystem resize max "$btrfs_mount" || true
            ${pkgs.btrfs-progs}/bin/btrfs filesystem sync "$btrfs_mount" || true
            ${pkgs.util-linux}/bin/umount "$btrfs_mount" || true
          fi
        fi
