...
in
{
  options.services.atc.tuning = {
    ...
    tailscaleInterface = mkOption {
      type = types.str;
      default = "tailscale0";
      description = "Interface name for Tailscale overlay connection";
    };

    clampMss = mkOption {
      type = types.int;
      default = 1240;
      description = "TCP MSS clamping target for Tailscale WireGuard overlay (MTU 1280 - 40)";
    };
  ...
  config = mkIf cfg.enable {
    boot.kernel.sysctl = {
      ...
      # 3. 启用 TCP 窗口缩放 (RFC 1323) 与高精度时间戳
      ...
      # 7. 控制 Socket 发送低水位，避免 FQ 队列暴涨 (Bufferbloat)
      "net.ipv4.tcp_notsent_lowat" = mkDefault 131072;

      # 8. PMTU 黑洞探测 (解决 Tailscale MTU 1280 丢弃 ICMP 的问题)
    ...
    # 强制在 Tailscale 虚拟网卡上实行 MSS Clamping
    networking.firewall.extraCommands = ''
      iptables -t mangle -A POSTROUTING -p tcp --tcp-flags SYN,RST SYN -o ${cfg.tailscaleInterface} -j TCPMSS --set-mss ${toString cfg.clampMss} || true
      iptables -t mangle -A PREROUTING -p tcp --tcp-flags SYN,RST SYN -i ${cfg.tailscaleInterface} -j TCPMSS --set-mss ${toString cfg.clampMss} || true
    '';
  };
}
